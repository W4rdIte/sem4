...
class Graph:
    ...
    def generate_prims_minimum_spanning_tree(self):
        ...
        for _ in range(self.size - 1):
            ...
            for neighbour_node in range(self.size):
                if (
                    not visited[neighbour_node]
                    and self.adjacency_matrix[min_node][neighbour_node] != 0
                    and self.adjacency_matrix[min_node][neighbour_node]
                    < cost[neighbour_node]
                ):
                    cost[neighbour_node] = self.adjacency_matrix[min_node][
                        neighbour_node
                    ]
                    previous[neighbour_node] = min_node
    ...
    def calculate_info_transmission_order(self, root):
        # Initialize necessary data structures
        rounds = [-1] * self.size  # to store the round number each node is reached
        visited = [False] * self.size

        # Perform BFS starting from the root
        queue = [(root, 0)]  # (current_node, current_round)
        visited[root] = True
        max_round = 0

        while queue:
            node, current_round = queue.pop(0)
            ...
            for neighbour_node in range(self.size):
                if (
                    not visited[neighbour_node]
                    and self.adjacency_matrix[node][neighbour_node] != 0
                ):
                    visited[neighbour_node] = True
                    queue.append((neighbour_node, current_round + 1))

        return rounds, max_round
