...
class Wire:
    ...
    class SignalPropagator:
        def __init__(
            self, symbol: str, position: int, tick_lifetime: int, wire: "Wire"
        ) -> None:
            self._symbol = symbol
        ...
        def _propagate_signal(self, position: int) -> None:
            if self._is_position_valid(position):
                self._wire.signal_groups[position].append(
                    self._wire.Signal(self._symbol, self._tick_lifetime)
                )
    ...
    def add_signal(
        ...
        self._signal_propagators.append(
            self.SignalPropagator(signal_symbol, device_position, tick_lifetime, self)
        )

    def add_jam_signal(self, device_position: int, tick_lifetime: int) -> None:
        self._signal_propagators.append(
            self.SignalPropagator(
                self.JAM_SIGNAL_SYMBOL, device_position, tick_lifetime, self
            )
        )
