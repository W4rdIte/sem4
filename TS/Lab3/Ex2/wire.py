...
class Wire:
    ...
    def tick(self) -> None:
        ...
        self._signal_groups = [
            [signal for signal in signals if signal.tick() or signal.is_active]
            for signals in self._signal_groups
        ]
        ...
        for position, signal_group in enumerate(self._signal_groups):
            for signal in signal_group:
                if signal.is_ready_to_propagate:
                    if (
                        signal.direction == self.SignalDirection.LEFT
                        or signal.direction == self.SignalDirection.BOTH
                    ) and self.is_position_valid(position - 1):
                        new_signals.append(
                            (
                                self.Signal(
                                    signal.symbol,
                    ...
                    if (
                        signal.direction == self.SignalDirection.RIGHT
                        or signal.direction == self.SignalDirection.BOTH
                    ) and self.is_position_valid(position + 1):
                        new_signals.append(
                            (
    ...
    def send_signal(
        self, device_position: int, signal_symbol: str, tick_lifetime: int
    ) -> None:
        self._signal_groups[device_position].append(
            self.Signal(signal_symbol, tick_lifetime, self.SignalDirection.BOTH)
        )

    def send_jam_signal(self, device_position: int, tick_lifetime: int) -> None:
        self._signal_groups[device_position].append(
            self.Signal(
                self.JAM_SIGNAL_SYMBOL, tick_lifetime, self.SignalDirection.BOTH
            )
        )

    def is_free(self, position: int, signal_symbol: str) -> bool:
        return (
            self._symbols[position] == self.DEFAULT_SIGNAL_SYMBOL
            or self._symbols[position] == signal_symbol
        )
