...
def are_same_key(ciphertext1, ciphertext2):
    if len(ciphertext1) != len(ciphertext2):
        return False

    # XOR dwóch kryptogramów
    xor_result = bytes([c1 ^ c2 for c1, c2 in zip(ciphertext1, ciphertext2)])

    # Sprawdzenie, czy wynik XOR wygląda na zaszyfrowany tekst ASCII
    try:
...
def test_rc4():
    ...
    for plaintext in plaintexts:
        ...
        print(
            f"Do ciphertext1 and ciphertext2 use the same key? {are_same_key(ciphertext1, ciphertext2)}"
        )
        print()
